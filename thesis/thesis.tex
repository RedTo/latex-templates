\documentclass[paper=a4,12pt,parskip=half,toc=listof,numbers=noendperiod, twoside]{scrartcl}
...
\usepackage{setspace}
%\usepackage{url}
\PassOptionsToPackage{hyphens}{url} %line breaks in urls
\usepackage{tabularx}
\usepackage{makecell}
\usepackage{multirow}
\usepackage{wrapfig}
...
\DeclareCaptionType{captionedEquation}[][]
\captionsetup[captionedEquation]{labelformat=empty}
%\usepackage{subcaption}
%\usepackage{picinpar}
\usepackage{tabu}
\usepackage{subfigure} %subfigure / plot images next to each other
...
\def\ThesisLocation{München}
\def\ThesisType{Master}
\def\ThesisPubDate{\today} % Change here to the date you are going to print your thesis
\def\ThesisFirstSupervisor{Prof. Dr. XX YY}
\def\ThesisSecondSupervisor{Prof. Dr. YY ZZ}
\def\ThesisExternalSupervisor{Dr. AA BB}
...
\newcommand{\subsubsubsubsection}[1]{\textbf{#1}\mbox{}\\}
...
\rehead{\textbf{\pagemark}}
\lohead{\textbf{\leftmark}}
\lehead{\textbf{\rightmark}}
...
\bibliographystyle{thesisplainnat} % author and year
